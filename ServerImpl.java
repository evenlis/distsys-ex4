...
public class ServerImpl extends UnicastRemoteObject implements Server
{
	...
	if((pendingLock = activeTransaction.getWaitingForResource())==null)
	    return;
	...
	if(transactions.contains(activeTransaction.getId())){
		...
		return;
	    int serverId = this.getTransactionOwner(transId);
	    Probe probe = new Probe(this, transactions, activeTransaction.getId(), serverId);
	    probe.start();
	}
    }
